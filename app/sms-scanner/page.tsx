...
export default function SMSScannerPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Attempt to get pre-computed response from Oracle
      let response = await Oracle.getScanResult(text, token);

      // If Oracle returns null (e.g. error in background fetch), fallback
      if (!response) {
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    ...
                    debounceRef.current = setTimeout(() => {
                      if (newText.length > 20) { // Only predict for meaningful text length
                        const token = localStorage.getItem('vsdp_token') || 'dummy_token';
