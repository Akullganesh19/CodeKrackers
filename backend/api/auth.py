...
async def send_otp(
    *,
    ...
    if (
        ...
        try:
            ...
            client.messages.create(
                body=f"VSDP Security Code: {otp_code}. Valid for 5 mins. Do not share.",
    ...
    if "@" in otp_in.identifier and settings.SENDGRID_API_KEY:
        try:
            body_txt = f"VSDP Security Code: {otp_code}. Valid 5m. Do not share."
            message = Mail(
                from_email=settings.FROM_EMAIL,
                to_emails=otp_in.identifier,
                subject="VSDP Security Code",
                plain_text_content=body_txt,
            )
