    ...
    if (
        ...
        try:
            ...
            client.messages.create(
                body=f"VSDP Code: {otp_code}. Valid for 5 mins. Do not share.",
    ...
    if "@" in otp_in.identifier and settings.SENDGRID_API_KEY:
        try:
            message = Mail(
                ...
                plain_text_content=f"VSDP code: {otp_code}. Valid 5min. Do not share.",
            )
    ...
    *,
    ...
    if not stored_code or otp_verify.code != stored_code:
        if user:
            ...
            db.commit()
        logger.warning("Auth failure: Invalid OTP attempt")
        raise HTTPException(
