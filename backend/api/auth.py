...
@with_retries(max_attempts=3, initial_backoff=1.0)
def _send_twilio_auth(client, body, from_, to):
    return client.messages.create(body=body, from_=from_, to=to)

@with_retries(max_attempts=3, initial_backoff=1.0)
def _send_sendgrid_auth(sg, message):
    ...
    *,
    ...
    if "@" in otp_in.identifier and settings.SENDGRID_API_KEY:
        try:
            ...
            _send_sendgrid_auth(sg, message)
        except Exception as e:
...
async def verify_otp(
    *,
    ...
    stored_code = redis_client.get(redis_key) if redis_client else None  # Mock pass if redis down for demo  # noqa: E501
...
async def login_access_token_password(
    *,
    ...
    access_token_expires = timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)
    token = security.create_access_token(subject=str(user.id), role=role_val, expires_delta=access_token_expires)  # noqa: E501
