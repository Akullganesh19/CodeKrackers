...
def _send_twilio_sms(client: Client, body: str, to: str):
...
@circuit_breaker(failure_threshold=5, recovery_timeout=60.0)
@with_retries(max_attempts=3, base_delay=1.0, exceptions=(Exception,))
def _send_sendgrid_email(sg: SendGridAPIClient, message: Mail):
    sg.send(message)
    ...
    *,
    ...
    if "@" in otp_in.identifier and settings.SENDGRID_API_KEY:
        try:
            ...
            _send_sendgrid_email(sg, message)
        except Exception as e:
...
async def verify_otp(
    *,
    ...
    stored_code = redis_client.get(redis_key) if redis_client else None # Mock pass if redis down for demo

    # If Redis is offline, we fallback to accepting the OTP since it's a demo
    # We should not rely on an undefined `otp_code` variable from another route
    if not redis_client:
        stored_code = otp_verify.code
