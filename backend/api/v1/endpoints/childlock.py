...
def create_child_profile(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422,
            detail=f"Invalid lock_mode. Options: {[m.value for m in ChildLockMode]}",
        )
    ...
    logger.info(
        "CHILD_PROFILE_CREATED id=%d parent=%d name=%s mode=%s",
        profile.id,
        current_user.id,
        name,
        mode.value,
...
def list_child_profiles(
    ...
    profiles = (
        db.query(ChildProfile).filter(ChildProfile.parent_id == current_user.id).all()
    )
    return [
        {
            "id": p.id,
            ...
            "lock_mode": (
                p.lock_mode.value if hasattr(p.lock_mode, "value") else p.lock_mode
            ),
...
def check_child_call(
    ...
    profile = (
        ...
        .filter(
            ChildProfile.id == profile_id, ChildProfile.parent_id == current_user.id
        )
        .first()
    )
    if not profile:
