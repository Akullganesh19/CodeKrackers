...
async def handle_account_locked(
    user_id: str, identifier: str, attempt_count: int, **kwargs
):
    """
    ...
    try:
        logger.info(
            f"Cross-system sync: Auth reported lockout for {identifier}. "
            "Creating Threat Intel record."
        )
        ...
        async with AsyncSessionLocal() as db:
            threat = Threat(
                ...
                raw_content=(
                    f"System blocked account due to {attempt_count} failed login "
                    "attempts. Potential account takeover or brute-force attack."
                ),
                extra_info={
                    "source": "auth_system",
            ...
            await db.commit()
            logger.info(
                f"Successfully recorded Threat for locked account {identifier}."
            )
