...
def handle_account_locked(user_email: str, failed_attempts: int, ip_address: str = "unknown", **kwargs):
    """
    ...
    """
    logger.info(f"Cross-system intelligence: Account locked for {user_email} after {failed_attempts} attempts. Adding to Threat Intelligence blacklist.")
    db = SessionLocal()
    try:
        ...
        auto_blacklist(
            ...
            reason=f"Auto-blacklisted due to {failed_attempts} failed login attempts from IP: {ip_address}",
            confidence=0.6,
