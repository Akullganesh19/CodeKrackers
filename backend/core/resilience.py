...
def with_retry_sync(max_attempts: int = 3, initial_backoff: float = 0.1, backoff_factor: float = 2.0, exceptions_to_retry: tuple = (Exception,)):
    """
        ...
        def wrapper(*args, **kwargs):
            ...
            while True:
                try:
                    ...
                    logger.warning(f"Function {func.__name__} failed (attempt {attempt}/{max_attempts}). Retrying in {backoff}s. Error: {e}")
                    time.sleep(backoff)
                    attempt += 1
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if self.state == "HALF_OPEN":
                    self.state = "CLOSED"
                    self.failure_count = 0
                    logger.info(f"Circuit breaker for {func.__name__} closed after successful test.")
                ...
                if self.state == "HALF_OPEN":
                    ...
                    self.state = "OPEN"
                    logger.warning(f"Circuit breaker for {func.__name__} reopened after test failure.")
