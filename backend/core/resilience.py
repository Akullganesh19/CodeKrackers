            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if attempt == max_attempts:
                        logger.error(
                            f"Action failed after {max_attempts} attempts: {e}"
                        )
                        raise

                    backoff = initial_backoff * (2 ** (attempt - 1))
                    logger.warning(
                        f"Attempt {attempt} failed, retrying in {backoff}s..."
                    )
        ...
        def wrapper(*args, **kwargs):
            ...
            if state == "OPEN":
                if now - last_failure_time >= cooldown_seconds:
                    ...
                    raise Exception(
                        f"Circuit for {func.__name__} is OPEN. Fast failing request."
                    )
            ...
            try:
                ...
                if state in ["HALF_OPEN", "CLOSED"]:
                    if failure_count > 0:
                        logger.info(
                            f"CircuitBreaker for {func.__name__} CLOSED. Resetting failure count."
                        )
                    failure_count = 0
                    state = "CLOSED"
                return result
                ...
                if state == "HALF_OPEN":
                    logger.error(
                        f"CircuitBreaker for {func.__name__} failed in HALF_OPEN. Re-opening."
                    )
                    state = "OPEN"
                elif state == "CLOSED" and failure_count >= failure_threshold:
                    logger.error(
                        f"CircuitBreaker for {func.__name__} tripped OPEN after {failure_count} failures."
                    )
                    state = "OPEN"
