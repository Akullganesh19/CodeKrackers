            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if attempt == max_attempts:
                        logger.error(f"Action '{func.__name__}' failed after {max_attempts} attempts. Error: {e}")
                        raise
                    logger.warning(f"Action '{func.__name__}' failed on attempt {attempt}/{max_attempts}. Retrying in {delay}s. Error: {e}")
                    time.sleep(delay)
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if attempt == max_attempts:
                        logger.error(f"Async action '{func.__name__}' failed after {max_attempts} attempts. Error: {e}")
                        raise
                    logger.warning(f"Async action '{func.__name__}' failed on attempt {attempt}/{max_attempts}. Retrying in {delay}s. Error: {e}")
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        ...
        def wrapper(*args, **kwargs) -> T:
            if not cb.can_execute():
                logger.error(f"Circuit breaker is OPEN. Skipping execution of '{func.__name__}'.")
