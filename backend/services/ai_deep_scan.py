...
@with_retry_sync(max_attempts=3, base_delay=0.2)
def call_groq_api(client, prompt, model):
...
@with_retry_sync(max_attempts=2, base_delay=0.1)
def check_ollama_local():
    return requests.get("http://localhost:11434", timeout=1)
...
def ai_deep_scan(content: str, source_type: str = "sms") -> Dict[str, Any]:
    ...
    try:
        ...
        chat_completion = call_groq_api(client, prompt, settings.GROQ_MODEL)
