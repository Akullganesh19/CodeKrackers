...
@CircuitBreaker(failure_threshold=2, recovery_timeout=30.0)
@with_retry_sync(max_attempts=3, backoff_factor=0.5)
def _check_ollama_gateway():
    response = requests.get("http://localhost:11434", timeout=1)
    response.raise_for_status()
    return response

@CircuitBreaker(failure_threshold=3, recovery_timeout=60.0)
@with_retry_sync(max_attempts=3, backoff_factor=1.0)
def _do_groq_request(client, prompt: str, source_type: str):
