...
def fallback_groq_scan(content: str, source_type: str = "sms", *args, **kwargs) -> Dict[str, Any]:
    """Fallback function if Groq Cloud API circuit breaker is open or fails after retries."""
    logger.warning("Using fallback for Groq API. Returning safe default.")
    return {"score_increase": 0.0, "reason": "Cloud AI degraded. Defaulting to safe.", "risk_factors": []}

@circuit_breaker(failure_threshold=3, recovery_timeout=60.0, fallback_func=fallback_groq_scan)
...
def call_groq_api(content: str, source_type: str) -> Dict[str, Any]:
    ...
    chat_completion = client.chat.completions.create(
        ...
        timeout=10.0 # Added timeout to avoid hanging
    )
