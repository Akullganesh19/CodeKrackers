...
class EvidenceChain:
    ...
    async def create_genesis_block(self, threat_id: uuid.UUID) -> Evidence:
        ...
        current_hash = self._generate_hash(str(previous_hash), dict(payload) if isinstance(payload, dict) else {}, str(timestamp))  # type: ignore
    ...
    async def verify_integrity(self, threat_id: uuid.UUID) -> dict:
        ...
        for i, block in enumerate(blocks):
            expected_hash = self._generate_hash(
                block.previous_hash,  # type: ignore
                block.payload,  # type: ignore
                block.timestamp.isoformat()  # type: ignore
            )
