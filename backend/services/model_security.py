...
def approve_model(db: Session, model_id: int, approved_by: str) -> Optional[ModelVersion]:
    ...
    ) if model.watermark_embedding else ""
...
class ExtractionDetector:
    ...
    def __init__(self):
        ...
        self._ip_input_hashes: Dict[str, Dict[str, int]
                                    ] = defaultdict(lambda: defaultdict(int))
        self._ip_model_queries: Dict[str, Dict[str, int]
                                     ] = defaultdict(lambda: defaultdict(int))
    ...
    def log_inference(
        ...
        api_key_hash = hashlib.sha256((api_key or "anonymous").encode()).hexdigest()[
            :16] if api_key else None
