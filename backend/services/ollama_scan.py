...
@with_retries(max_attempts=3, base_delay=0.2, max_delay=2.0)
def _call_ollama(payload: Dict[str, Any]) -> requests.Response:
    """
    Internal helper to call Ollama with resilience mechanisms.
    """
...
def ollama_deep_scan(content: str, source_type: str = "sms") -> Dict[str, Any]:
    ...
    # We purposefully do not catch the exception here so that callers (like ai_deep_scan)
    # can fallback gracefully when the circuit is open or retries fail.
