...
SPAM_PATTERNS: List[str] = [
    ...
    # Added for live voice transcripts
    r"(?:arrest|police|cbi|fir|warrant).*(?:immediately|now|urgent)",
]
...
def check_spam(
    ...
    if user_filter and user_filter.whitelisted_numbers:
        ...
        for wl in user_filter.whitelisted_numbers:
            if re.sub(r"[\s\-()]", "", wl) in clean_number or clean_number in re.sub(r"[\s\-()]", "", wl):
                breakdown.append({"factor": "Whitelisted contact",
                                 "points": "+0.0", "type": "positive"})
                return _result(SpamAction.ALLOW, 0.0, breakdown, user_id, phone_number, spam_type, db, content)

    breakdown.append({"factor": "Not on whitelist",
                     "points": "+0.0", "type": "neutral"})
    ...
    if report_count >= 3:
        points = 0.25
        spam_score += points
        breakdown.append({"factor": f"Reported by {report_count} users",
                         "points": f"+{points}", "type": "negative"})
    elif report_count > 0:
        points = round(report_count * 0.1, 2)
        spam_score += points
        breakdown.append({"factor": f"Reported by {report_count} user(s)",
                         "points": f"+{points}", "type": "negative"})
    ...
    if unique_users_called and unique_users_called >= 50:
        points = 0.6
        spam_score += points
        breakdown.append({"factor": f"SIM Farm detected: called {unique_users_called} users in 1hr",
                         "points": f"+{points}", "type": "critical"})
    ...
    if bl:
        ...
        spam_score += points
        breakdown.append({"factor": f"Global blacklist hit (confidence: {bl.confidence:.0%})",
                         "points": f"+{points}", "type": "critical"})
    else:
        breakdown.append({"factor": "Not on global blacklist",
                         "points": "+0.0", "type": "positive"})
    ...
    if carrier_info.get("is_voip"):
        points = 0.3
        spam_score += points
        breakdown.append({"factor": "VoIP number detected",
                         "points": f"+{points}", "type": "negative"})
        if user_filter and user_filter.block_voip:
            breakdown.append({"factor": "User filter: Block VoIP",
                             "points": "BLOCK", "type": "critical"})
    ...
    if phone_info.get("country_code") and phone_info["country_code"] != "IN":
        ...
        spam_score += points
        breakdown.append(
            {"factor": f"International number ({phone_info['country_code']})", "points": f"+{points}", "type": "negative"})
        if user_filter and user_filter.block_international:
            breakdown.append({"factor": "User filter: Block International",
                             "points": "BLOCK", "type": "critical"})
    ...
    if content:
        ...
        for pattern in SPAM_PATTERNS:
            if re.search(pattern, content_lower):
                ...
                spam_score += points
                breakdown.append(
                    {"factor": f"Scam pattern detected: {pattern[:20]}...", "points": f"+{points}", "type": "negative"})
        ...
        for indicator in ROBOCALL_INDICATORS:
            if indicator in content_lower:
                points = 0.2
                spam_score += points
                breakdown.append({"factor": f"Robocall indicator: '{indicator}'",
                                 "points": f"+{points}", "type": "negative"})
        ...
        if re.search(r"https?://|www\.|bit\.ly|tinyurl", content_lower):
            points = 0.1
            spam_score += points
            breakdown.append({"factor": "Unsolicited short URL detected",
                             "points": f"+{points}", "type": "negative"})
        ...
        if found_urgency:
            points = 0.15
            spam_score += points
            breakdown.append({"factor": f"Urgency words: {', '.join(found_urgency)}",
                             "points": f"+{points}", "type": "negative"})
        ...
        if user_filter and user_filter.blocked_keywords:
            for kw in user_filter.blocked_keywords:
                if kw.lower() in content_lower:
                    ...
                    spam_score += points
                    breakdown.append(
                        {"factor": f"Custom blocked keyword: '{kw}'", "points": f"+{points}", "type": "negative"})
        ...
        if 0.1 <= spam_score < 0.9:
            ai_result = ai_deep_scan(content, "sms" if spam_type ==
                                     SpamType.SMS else "call_transcript")
    ...
    if user_filter and user_filter.block_unknown_callers and spam_type == SpamType.CALL:
        if not carrier_info.get("name") or carrier_info["name"] == "Unknown":
            points = 0.2
            spam_score += points
            breakdown.append({"factor": "Unknown caller blocked by filter",
                             "points": f"+{points}", "type": "negative"})
    ...
    if user_filter and user_filter.auto_block_reported_spam and report_count >= 2:
        action = SpamAction.BLOCK
        breakdown.append({"factor": "User filter: Auto-block reported",
                         "points": "BLOCK", "type": "critical"})
...
def _result(
    ...
    critical_factors = [b["factor"]
                        for b in breakdown if b["type"] in ("negative", "critical")]
    ...
    if action == SpamAction.BLOCK:
        logger.warning("SPAM_BLOCKED phone=%s score=%.2f reason=%s",
                       phone, score, reason_str[:80])
        ...
        logger.info("SPAM_CHECK phone=%s action=%s score=%.2f",
                    phone, action.value, score)
