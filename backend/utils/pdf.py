...
def generate_fir_pdf(fir_id: int, officer_name: str, threat_details: dict) -> tuple[str, str]:
    ...
    elements.append(Paragraph(
        f"Official Forensic Report | Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal']))
    ...
    elements.append(Paragraph(threat_details.get(
        "content", "No content provided."), styles['Normal']))
