...
def generate_fir_pdf(data: dict, output_path: str):
    ...
    # Header
    content.append(
        Paragraph("VSDP - Vishing & Smishing Defense Platform", header_style))
    content.append(
        Paragraph(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", header_style))
    ...
    # Legal Sections
    content.append(
        Paragraph("<b>Relevant Legal Sections (IPC/IT Act):</b>", styles['Heading3']))
    ...
    # Incident Details
    content.append(
        Paragraph("<b>Incident Description / Raw Evidence:</b>", styles['Heading3']))
    content.append(
        Paragraph(data.get("raw_content", "No content captured."), styles['Normal']))
