...
def generate_admin_report_pdf(data: dict, output_path: str):
    ...
    elements.append(
        Paragraph(
            f"Filters Applied: Month={filter_info['month'] or 'All'}, Year={filter_info['year'] or 'All'}",
            styles["Normal"],
        )
    )
    ...
    type_data = [["Threat Type", "Incidents"]] + [
        [t.capitalize(), str(c)] for t, c in data["stats"]["threats_by_type"].items()
    ]
    ...
    sev_data = [["Severity Level", "Count"]] + [
        [s.upper(), str(c)] for s, c in data["stats"]["threats_by_severity"].items()
    ]
    t_sev = Table(sev_data, colWidths=[200, 100])
    t_sev.setStyle(TableStyle([("GRID", (0, 0), (-1, -1), 0.5, colors.grey)]))
    elements.append(t_sev)
    elements.append(Spacer(1, 20))

    # 7-Day Incumbency Trend
    elements.append(Paragraph("7-Day Operational Trend", styles["Heading3"]))
    trend_data = [["Date", "New Threats Detected"]] + [
        [d["date"], str(d["count"])] for d in data["visualization"]["threat_trend_7d"]
    ]
